...
- (id)initWithRootViewControllerURL:(NSURL *)url withQuery:(NSDictionary*)query{
    self = [super init];
    if (self) {
        UMViewController *rootVC = nil;
        if (nil == query) {
            rootVC = [self viewControllerForURL:url withQuery:nil];
        }else{
            rootVC = [self viewControllerForURL:url withQuery:query];
        }
...
{
    return [self initWithRootViewControllerURL:url withQuery:nil];
//    self = [super init];
//    if (self) {
