...
var $O = {
    ...
    putLocalUserInfo: function(userinfo) {
        with(utilityNames) {
            var str = JsViewUtility.putLocalUserInfo(userinfo);
        }
    },
    // 缓存
    ...
    getDecorateConfig: function() {
        with(utilityNames) {
            var decorateConfig = JsViewUtility.getDecorateConfig();
        }
        return String(decorateConfig);
    },
    getPageId: function(params) {
        with(utilityNames) {
            var page_id = JsViewUtility.getPageId();
        }
        return page_id;
    },
    getLocalUserInfo: function() {
        with(utilityNames) {
            var userInfo = JsViewUtility.getLocalUserInfo();
        }
        return String(userInfo);
    },
    getPushToken: function() {
        with(utilityNames) {
            var pushToken = JsViewUtility.getPushToken();
        }
        return String(pushToken);
    },
    isLogin: function() {
        with(utilityNames) {
            var login = JsViewUtility.isLogin();
        }
        return Boolean(login);
    },
    getAppVersion: function() {
        with(utilityNames) {
            var versionNum = JsViewUtility.getAppVersion();
        }
        return parseFloat(versionNum);
    },
    getRemoteVersion: function() {
        with(utilityNames) {
            var netVersionNum = JsViewUtility.getRemoteVersion();
        }
        return parseFloat(netVersionNum);
    },
    checkNewVersion: function() {
        with(utilityNames) {
            var isNewVersion = JsViewUtility.checkNewVersion();
        }
        return Boolean(isNewVersion);
    },
    confirmDownloadNewVersion: function() {
        with(utilityNames) {
            JsViewUtility.confirmDownloadNewVersion();
        }
    },
    getUserName: function() {
        with(utilityNames) {
            var name = JsViewUtility.getUserName();
        }
        return String(name);
    },
    saveUserName: function(userName) {
        with(utilityNames) {
            JsViewUtility.saveUserName(userName);
        }
    },
    clearUserName: function() {
        with(utilityNames) {
            JsViewUtility.clearUserName();
        }
    },
    ...
    clearCache: function() {
        with(utilityNames) {
            JsUtility.clearCache();
        }
    },
    clearInflated: function() {
        with(utilityNames) {
            JsViewUtility.clearInflated();
        }
    },
    ...
    callWidgetMethod: function(controlId, methodName, param1, param2) {
        with(utilityNames) {
            var str = JsViewUtility.callWidgetMethod(controlId, methodName, param1, param2);
        }
    },
    getValuePurpose: function(control_id, desc, bundle_string) {
        with(utilityNames) {
            var str = JsViewUtility.getValuePurpose(control_id, desc, bundle_string);
        }
        return String(str);
    },
    ...
    getPageParam: function(key) {
        with(utilityNames) {
            var str = JsViewUtility.getPageParam(key);
        }
        return String(str);
    },
    ...
    inflateApp: function(projectId) {
        with(utilityNames) {
            JsViewUtility.inflateApp(projectId);
        }
    },
    ...
    getLocation: function(locationgTag) {
        with(utilityNames) {
            JsViewUtility.getLocation(locationgTag);
        }
    },
    ...
    getCache: function(sort1, sort2, md5) {
        ...
        var para = Object.toJSON(obj)
        with(utilityNames) {
            var str = JsViewUtility.getCache(para);
        }
        return String(str).evalJSON().result;
    },
    // 移除cache
    removeCaches: function(sort1, sort2, md5) {
        ...
        var para = Object.toJSON(obj)
        with(utilityNames) {
            JsViewUtility.removeCaches(para);
        }
    },
    app_callApi: function(param) {
      var para = JSON.stringify(param);
        with(utilityNames) {
          var str = JsViewUtility.app_callApi(para);
        }
...
//		$L.D("handle","match:"+match);
  ...
  evalMethod : function( control_id , event_name , params , scopeid ){
    var tag = "control_id-"+control_id+" - "+event_name;
    try {
  		$L.D(tag,"input :"+params);
      $C.scopeid = scopeid;
      if (typeof root[control_id] == "undefind" || root[control_id] == null )
        return $L.D(tag,"cant find control :"+control_id);
      if (typeof root[control_id][event_name] == "undefind" || root[control_id][event_name] == null )
        return $L.D(tag,"cant find event :" + control_id + " - "+ event_name);
      var res = root[control_id][event_name](params);
//      var res = eval("func("+params+")");
      if (typeof res != "string")
			  res = Object.toJSON( res ) || "";
			$L.D(tag,"core_android output:"+res);
		} catch(e) {
...
var LogHandler = Class.create({
	...
	androidLog : function(params){
		with(logNames){
			switch (params.type){
				case "Info":
					LogUtil.i(params.tag,params.msg);
				break;
				case "Debug":
					LogUtil.d(params.tag,params.msg);
				break;
				case "Warning": 
					LogUtil.w(params.tag,params.msg);
				break;
				case "Error":
					LogUtil.e(params.tag,params.msg);
				break;
			}
		}
	},
...
logNames.importClass(com.ecloudiot.framework.utility.LogUtil);
